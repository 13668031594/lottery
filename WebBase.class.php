...
class WebBase extends Object
{
    ...
    //北京PK10_2
    public function BJpk10_2(&$actionNo, &$actionTime, $time = null)
    {
        $this->setTimeNo($actionTime, $time);
    ...
    //北京快乐8_2
    public function bjkl8_2(&$actionNo, &$actionTime, $time = null)
    {
        $this->setTimeNo($actionTime, $time);
    ...
    //反馈success
    public function json_success($message = '操作成功', $code = 0, $test = [], $other = [])
    {
        $result = self::json_array('success', $message, $code, $test, $other);

        exit($result);
    }

    //反馈failes
    public function json_fails($message = '未知错误', $code = 0, $test = [], $other = [])
    {
        $result = self::json_array('files', $message, $code, $test, $other);

        exit($result);
    }

    //json反馈字符串
    public function json_array($status = 'unKnow', $message = '', $code = 0, $test = [], $other = [])
    {
        $array = [
            'status' => $status,
            'code' => $code,
    ...
    //跳转路由反馈
    public function json_display($array = [], $other = [])
    {
        $other['user'] = $this->user;

        $args = func_get_args();
        $argc = func_num_args();
//        if ($argc == 0) self::json_fails('Template is null');
...
//        unset($args[0]);
        ...
        $other['args'] = $args;
        $other['argc'] = $argc;
        $other['settings'] = isset($this->settings) ? $this->settings : null;
        $other['action'] = $this->action;

        self::json_success(null, 0, $array, $other);
    }
}