...
//            return $this->user['username'] . '您好：<br>' . '您投注的' . $sscname . '第' . $code['actionNo'] . '期<br>金额为：' . $amount . '元<br>已投注成功！';
            parent::json_success($this->user['username'] . '您好：<br>' . '您投注的' . $sscname . '第' . $code['actionNo'] . '期<br>金额为：' . $amount . '元<br>已投注成功！');
        } catch (Exception $e) {
            $this->rollBack();
//            throw $e;
    ...
    public final function postlhcData()
    {
        ...
        $this->getSystemSettings();
        if ($this->settings['switchBuy'] == 0)
            parent::json_fails('游戏玩法已停止投注');
        if (count($codes) == 0)
            parent::json_fails('请先买单再提交投注');
        //玩法开启
        if (!$this->getValue("select enable from {$this->prename}played where id=?", intval($para['playedId'])))
            parent::json_fails('游戏玩法已停,请刷新再投(2)');
        //检查时间 期数
        $ftime = $this->getTypeFtime(intval($para['type'])); //封单时间
        $actionTime = $this->getGameActionTime(intval($para['type'])); //当期时间
        $actionNo = $this->getGameActionNo(intval($para['type'])); //当期期数
        ...
        $userAmount = $this->getValue("select coin from {$this->prename}members where uid={$this->user['uid']}");
        if ($userAmount < $amount)
            parent::json_fails('您的可用资金不足，请充值(3)');
        // 开始事物处理
        $this->beginTransaction();
        try {
            foreach ($codes as $code) {
...
//        if ($this->settings['switchZDLBuy'] == 0 && ($this->user['parents'] == $this->user['uid'])) parent::json_fails('总代理不能买单！');
        ...
        //检查时间 期数
        if ($return['kjTime'] < $this->time) parent::json_fails('提交数据出错,请刷新再投');
        ...
        if ($actionTime != $return['kjTime']) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        if ($actionNo != $return['actionNo']) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        if ($actionTime - $ftime < $this->time) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        // 查检每注的赔率是否正常
        $this->getPlayeds();
        ...
        if ($actionTime2 != $return['kjTime']) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        if ($actionNo2 != $return['actionNo']) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        if ($actionTime - $ftime2 < $this->time) parent::json_fails('投注失败：你投注第' . $return['actionNo'] . '已过购买时间');
        $played = $this->playeds[$return['playedId']];
        //检查开启
        if (!$played['enable']) parent::json_fails('游戏玩法组已停,请刷新再投');
        //检查ID
        if ($played['groupId'] != $return['playedGroup']) parent::json_fails('提交数据出错，请重新投注');
        if ($played['id'] != $return['playedId']) parent::json_fails('提交数据出错，请重新投注');
        //检查赔率
        ...
        if ($return['bonusProp'] > $played['bonusProp']) parent::json_fails('提交奖金大于最大奖金，请重新投注');
        if ($return['bonusProp'] < $played['bonusPropBase']) parent::json_fails('提交奖金小于最小奖金，请重新投注');
...
//			if(floatval($return['fanDian'])>floatval($this->user['fanDian']) || floatval($return['fanDian'])>floatval($this->settings['fanDianMax'])) parent::json_fails('提交返点出错，请重新投注');
        ...
        if (in_array($return['playedId'], $arr4id)) {
            if (!in_array($return['weiShu'], $arr4)) parent::json_fails('提交数据出错，请重新投注');
        }
        if (in_array($return['playedId'], $arr3id)) {
            if (!in_array($return['weiShu'], $arr3)) parent::json_fails('提交数据出错，请重新投注');
        }
        if (in_array($return['playedId'], $arr2id)) {
            if (!in_array($return['weiShu'], $arr2)) parent::json_fails('提交数据出错，请重新投注');
        }
        //检查模式
        $mosi = array();
        ...
        if (!in_array($return['mode'], $mosi)) parent::json_fails('投注模式出错，请重新投注');
        // 检查注数
        if ($return['actionNum'] < 1) parent::json_fails('注数不能小于1，请重新投注');
        if ($betCountFun = $played['betCountFun']) {
        ...
        if ($return['actionNum'] > $maxcount) parent::json_fails('注数超过玩法:' . $playedname . '  最高注数:' . $maxcount . '注,请重新投注!');
        //最低消费金额计算
...
//                var_dump($para);
        ...
        if ($actionNo != $return['actionNo']) parent::json_fails('投注失败：你投注第' . $code['actionNo'] . '已过购买时间');
        ...
        // 查询用户可用资金
        $userAmount = $this->getValue("select coin from {$this->prename}members where uid={$this->user['uid']}");
        if ($userAmount < $amount) parent::json_fails('您的可用资金不足，是否充值？');
        // 开始事物处理
        $this->beginTransaction();
        try {
    ...
    public final function deleteCode($id)
    {
        ...
        try {
            ...
            if (!$data = $this->getRow($sql, $id))
                parent::json_fails('找不到定单。');
            if ($data['isDelete'])
                parent::json_fails('这单子已经撤单过了。');
            if ($data['uid'] != $this->user['uid'])
                parent::json_fails('这单子不是您的，您不能撤单。'); // 可考虑管理员能给用户撤单情况
            if ($data['kjTime'] <= $this->time)
                parent::json_fails('已经开奖，不能撤单');
            if ($data['lotteryNo'])
                parent::json_fails('已经开奖，不能撤单');
    ...
    public final function deleteAllCode($id)
    {
        ...
        try {
            ...
            foreach ($datas as $data) {
                if ($data['isDelete'])
                    parent::json_fails('里面有单子已经撤单过了。');
                if ($data['uid'] != $this->user['uid'])
                    parent::json_fails('里面有单子不是您的，您不能撤单。');
                if ($data['kjTime'] < $this->time)
                    parent::json_fails('里面有单子已经开奖，不能撤单');
                if ($data['lotteryNo'])
                    parent::json_fails('已经开奖，不能撤单');
                if ($data['type'] == 34 || $data['type'] == 77)
                    parent::json_fails('对不起，六合彩不能撤单');
                // 冻结时间后不能撤单
                $this->getTypes();
                if ($data['kjTime'] - $this->types[$data['type']]['data_ftime'] < $this->time)
                    parent::json_fails('里面有单子已经结冻，不能撤单');
            }
            //数据处理
            foreach ($datas as $data) {
