...
//            throw new Exception('<font color="#FF0000">不允许空密码登录');
        ...
        if (!$vcode) {
            //throw new Exception('<font color="#FF0000">请输入验证码');
        }
        $sql = "select * from {$this->prename}members where isDelete=0 and admin=0 and username=?";
        if (!$user = $this->getRow($sql, $username)) {
//            throw new Exception('<font color="#FF0000">用户名或密码不正确');
            parent::json_fails('用户名或密码不正确');
        }
        if (md5($password) != $user['password']) {
//            throw new Exception('<font color="#FF0000">密码不正确');
            parent::json_fails('密码不正确');
        }
        if ($vcode != $_SESSION[$this->vcodeSessionName]) {
//            throw new Exception('<font color="#FF0000">验证码错误，请重新输入');
            parent::json_fails('验证码错误，请重新输入');
        }
        if (!$user['enable']) {
//            throw new Exception('<font color="#FF0000">您的帐号被冻结，请联系管理员。');
            parent::json_fails('您的帐号被冻结，请联系管理员。');
        }
        $session = array(
            'uid' => $user['uid'],
...
//        if (!ctype_digit($para['amount'])) throw new Exception('销量包含非法字符，请填入纯数字');
        if (!ctype_digit($para['amount'])) parent::json_fails('销量包含非法字符，请填入纯数字');
//        if (!ctype_digit($para['qq'])) throw new Exception('QQ号码包含非法字符');
        if (!ctype_digit($para['qq'])) parent::json_fails('QQ号码包含非法字符');
//        if (strlen($para['name']) > 40) throw new Exception('平台名请不要超过40个字符');
        if (strlen($para['name']) > 40) parent::json_fails('平台名请不要超过40个字符');

        $para['time'] = $this->time;
        $para['ip'] = $this->ip(true);
//        if (!$this->insertRow($this->prename . 'inagent', $para)) throw new Exception('提交出错，请重试!');
        if (!$this->insertRow($this->prename . 'inagent', $para)) parent::json_fails('提交出错，请重试!');

//        return '申请信息已成功提交';
        parent::json_success('申请信息已成功提交');
    }
...
//                        $this->display('team/register.php');
                        ...
                        parent::json_display(['userxxx' => $userxxx]);
...
//        if (!$_POST) throw new Exception('注册失败，请重新操作');
        if (!$_POST) parent::json_fails('注册失败，请重新操作');
...
//        if (strlen($user) < 5 || strlen($user) > 15) throw new Exception('帐号为5-15位,请重新输入');
        if (strlen($user) < 5 || strlen($user) > 15) parent::json_fails('帐号为5-15位,请重新输入');
//        if (!ctype_alnum($user)) throw new Exception('帐号包含非法字符');
        if (!ctype_alnum($user)) parent::json_fails('帐号包含非法字符');
//        if (strlen($password) < 6) throw new Exception('密码至少6位');
        if (strlen($password) < 6) parent::json_fails('密码至少6位');
//        if (strlen($qq) < 6 || strlen($qq) > 11) throw new Exception('QQ号为6-11位,请重新输入');
        if (strlen($qq) < 6 || strlen($qq) > 11) parent::json_fails('QQ号为6-11位,请重新输入');
        if (!ctype_digit($qq)) parent::json_fails('QQ包含非法字符');

        if (strtolower($vcode) != $_SESSION[$this->vcodeSessionName]) {
//            throw new Exception('验证码不正确。');
            parent::json_fails('验证码不正确。');
        }
...
//        if (!$codec) throw new Exception('链接错误');
        if (!$codec) parent::json_fails('链接错误');
...
//            throw new Exception('该链接已失效，请联系您的上级重新索取注册链接！！');
            parent::json_fails('该链接已失效，请联系您的上级重新索取注册链接！！');
        } else {
            $para = array(
                'username' => $user,
                'type' => $link['type'],
            ...
            try {
                ...
                if ($this->insertRow($this->prename . 'members', $para)) {
                    ...
                    $this->commit();
//                    return '注册成功';
                    parent::json_success('注册成功');
                } else {
//                    throw new Exception('注册失败');
                    parent::json_fails('注册失败');
                }
            } catch (Exception $e) {
//                $this->rollBack();
//                throw $e;
