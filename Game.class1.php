...
//		echo json_encode($actionNo);
	...
	public final function postCode(){
		...
		if(count($codes)==0) parent::json_fails('请先选择号码再提交投注');
		//检查时间 期数
		if($para['kjTime']<$this->time) parent::json_fails('提交数据出错,请刷新再投');
		...
		if($actionTime!=$para['kjTime'])  parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
		if($actionNo!=$para['actionNo'])  parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
		if($actionTime-$ftime<$this->time) parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
		// 查检每注的赔率是否正常
		$this->getPlayeds();
		foreach($codes as $code){
            ...
            if($actionTime2!=$para['kjTime'])  parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
		    if($actionNo2!=$para['actionNo'])  parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
		    if($actionTime-$ftime2<$this->time) parent::json_fails('投注失败：你投注第'.$para['actionNo'].'已过购买时间');
			$played=$this->playeds[$code['playedId']];
			//检查开启
			if(!$played['enable']) parent::json_fails('游戏玩法组已停,请刷新再投');
            //检查ID
            if($played['groupId']!=$code['playedGroup']) parent::json_fails('提交数据出错，请重新投注');
			if($played['id']!=$code['playedId']) parent::json_fails('提交数据出错，请重新投注');
			//检查赔率
			...
			if($code['bonusProp']<$played['bonusPropBase']) parent::json_fails('提交奖金小于最小奖金，请重新投注');
			...
			if(round($chkBonus)!=round($code['bonusProp'])) parent::json_fails('提交奖金出错，请重新投注');
			//检查返点
			if(floatval($code['fanDian'])>floatval($this->user['fanDian']) || floatval($code['fanDian'])>floatval($this->settings['fanDianMax'])) parent::json_fails('提交返点出错，请重新投注');
			//检查倍数
			if(intval($code['beiShu'])<1) parent::json_fails('倍数只能为大于1正整数');
			//检查位数
			if(in_array($code['playedId'],$arr4id)){
				if(!in_array($code['weiShu'],$arr4)) parent::json_fails('提交数据出错，请重新投注');
			}
			if(in_array($code['playedId'],$arr3id)){
				if(!in_array($code['weiShu'],$arr3)) parent::json_fails('提交数据出错，请重新投注');
			}
			if(in_array($code['playedId'],$arr2id)){
				if(!in_array($code['weiShu'],$arr2)) parent::json_fails('提交数据出错，请重新投注');
			}
			//检查模式
			$mosi=array();
			...
			if(!in_array($code['mode'],$mosi)) parent::json_fails('投注模式出错，请重新投注');
			// 检查注数
			if($code['actionNum']<1) parent::json_fails('注数不能小于1，请重新投注');
			if($betCountFun=$played['betCountFun']){
				if($code['actionNum']!=Bet::$betCountFun($code['actionData'])) parent::json_fails('下单失败,您投注号码不符合投注规则，请重新投注'.Bet::$betCountFun($code['actionData']));
			}
			//最大注数检查
            $maxcount=$this->getmaxcount($code['playedId']);
			$playedname=$this->getplayedname($code['playedId']);
            if($code['actionNum']>$maxcount) parent::json_fails('注数超过玩法:'.$playedname.'  最高注数:'.$maxcount.'注,请重新投注!');
			//最低消费金额计算
		...
		if($zhuihao=$_POST['zhuiHao']){
			...
			foreach(explode(';', $zhuihao) as $var){
				...
				if($code['kjTime']<$actionTime) parent::json_fails('请勿擅改开奖时间！');
				...
				if($code['actionNo'] != $ano['actionNo']){
					...
					if($dt2<$dt1 || ($dt2==$dt1 && $b2<$b1)){
						...
						parent::json_fails('投注失败2：您追投注的第'.$ano['actionNo'].'期已经过购买时间！');
					}
				}
				if(strtotime($ano['actionTime'])-$ftime<$this->time) parent::json_fails('投注失败3：你追号投注第'.$code['actionNo'].'已过购买时间');
            ...
            if($actionNo!=$code['actionNo'])  parent::json_fails('投注失败：你投注第'.$code['actionNo'].'已过购买时间');
		...
		//最低消费金额检查
		if($amount<$mincoin) parent::json_fails('本次投注方案总共:'.$allNum.'注,最低消费金额:'.$mincoin.'元,请重新投注!');
		// 查询用户可用资金
...
//			throw $e;
	...
	public final function postlhcData(){
		...
		$this->getSystemSettings();
		if($this->settings['switchBuy']==0) parent::json_fails('游戏玩法已停止投注');
		if(count($codes)==0) parent::json_fails('请先买单再提交投注');
		//玩法开启
		if(!$this->getValue("select enable from {$this->prename}played where id=?",intval($para['playedId']))) parent::json_fails('游戏玩法已停,请刷新再投');
		//检查时间 期数
		$ftime=$this->getTypeFtime(intval($para['type']));  //封单时间
		$actionTime=$this->getGameActionTime(intval($para['type']));  //当期时间
		$actionNo=$this->getGameActionNo(intval($para['type']));  //当期期数
		...
		foreach($codes as $i=>$code){
			...
			//检查返点
			if($code['fanDian']!=0) parent::json_fails('请勿改包！');
			$code['fanDian']=0;
			$code['mode']=1.00;
			//检查倍数
			if(intval($code['beiShu'])<1) parent::json_fails('倍数只能为大于1正整数');
...
//		$this->display('index/inc_game_order_history.php');
	...
	public final function deleteCode($id){
		...
		try{
			...
			if(!$data=$this->getRow($sql, $id)) parent::json_fails('找不到定单。');
			if($data['isDelete']) parent::json_fails('这单子已经撤单过了。');
			if($data['uid']!=$this->user['uid']) parent::json_fails('这单子不是您的，您不能撤单。');		// 可考虑管理员能给用户撤单情况
			if($data['kjTime']<=$this->time) parent::json_fails('已经开奖，不能撤单');
			if($data['lotteryNo']) parent::json_fails('已经开奖，不能撤单');
			if($data['type']==34) parent::json_fails('对不起，六合彩不能撤单');

			// 冻结时间后不能撤单
			$this->getTypes();
			$ftime=$this->getTypeFtime($data['type']);
			if($data['kjTime']-$ftime<$this->time) parent::json_fails('这期已经结冻，不能撤单');
...
//			throw $e;
	...
	public final function deleteAllCode($id){
		$amount=0;
		if($id){$id=str_replace('-',',',$id);}else{parent::json_fails('请选择要单号。');}
		$this->beginTransaction();
		try{
			$sql="select * from {$this->prename}bets where id in({$id})";
			if(!$datas=$this->getRows($sql)) parent::json_fails('找不到定单。');
			//数据判断
			foreach($datas as $data){
				if($data['isDelete']) parent::json_fails('里面有单子已经撤单过了。');
				if($data['uid']!=$this->user['uid']) parent::json_fails('里面有单子不是您的，您不能撤单。');
				if($data['kjTime']<$this->time) parent::json_fails('里面有单子已经开奖，不能撤单');
				if($data['lotteryNo']) parent::json_fails('已经开奖，不能撤单');
				if($data['type']==34) parent::json_fails('对不起，六合彩不能撤单');
				// 冻结时间后不能撤单
				$this->getTypes();
				if($data['kjTime']-$this->types[$data['type']]['data_ftime']<$this->time) parent::json_fails('里面有单子已经结冻，不能撤单');
			}
			//数据处理
			foreach($datas as $data){
